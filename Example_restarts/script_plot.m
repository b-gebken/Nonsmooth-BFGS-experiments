...
xline(reset_inds-1,'k--','LineWidth',1.25)
...
legend(h1,{'$t_k$'},...
    'Interpreter','latex','FontSize',15,'Position',[0.6873 0.1683 0.0803 0.0667]);
...
% Subfigure -----------------------------------------------------------
    ...
    bk_ax = axes('position',[.23 .15 .272 .320]);
    set(gca,'XColor','none','YColor','none')

    ax = axes('position',[.2 .2 .4 .27]);

    min_ind = 1280-5;
    max_ind = 1280+5;
    zoom_inds = min_ind:max_ind;

    plot(zoom_inds-1,log10(t_arr(zoom_inds)),'k.','MarkerSize',11,'LineWidth',lw);
    hold on
    xline(reset_inds(reset_inds >= min_ind & reset_inds <= max_ind)-1,'k--','LineWidth',1.25);

    xlim([min_ind-1-0.5,max_ind-1+0.5])
    ylim([-15,-1.0])
    axis square
    grid on
    ax.ClippingStyle = "rectangle";
